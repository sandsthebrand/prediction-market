...
class TestStatsFields:
    async def test_stats_has_no_open_positions_key(self, db):
        ...
        assert "open_positions" not in engine.stats()
...
class TestPhase0BaselineSnapshot:
    ...
    async def test_baseline_snapshot_captures_pair_count(self, db):
        ...
        for i in range(2):
            ...
            await db.execute(
                "INSERT OR IGNORE INTO markets (id, platform, platform_id, title, status, created_at, updated_at) VALUES (?, 'kalshi', ?, 'T', 'open', ?, ?)",
                (f"k{i}", f"k{i}", now, now),
            )
            await db.execute(
                "INSERT OR IGNORE INTO market_pairs (id, market_id_a, market_id_b, pair_type, created_at, updated_at) VALUES (?, ?, ?, 'cross_platform', ?, ?)",
    ...
    async def test_baseline_captures_strategy_pnl(self, db):
        ...
        await db.execute(
            "INSERT OR IGNORE INTO signals (id, violation_id, strategy, signal_type, market_id_a, model_edge, kelly_fraction, position_size_a, total_capital_at_risk, status, fired_at, updated_at) VALUES ('sig1', NULL, 'P1_cross_market_arb', 'arb_pair', 'tm', 0.05, 0.10, 5.0, 10.0, 'fired', ?, ?)",
            (now, now),
        )
        await db.execute(
            "INSERT INTO trade_outcomes (id, signal_id, strategy, violation_id, market_id_a, predicted_edge, predicted_pnl, actual_pnl, fees_total, edge_captured_pct, signal_to_fill_ms, holding_period_ms, spread_at_signal, volume_at_signal, liquidity_at_signal, resolved_at, created_at) VALUES ('to1', 'sig1', 'P1_cross_market_arb', NULL, 'tm', 0.05, 0.25, 10.0, 0.10, 100.0, 50, 5000, 0.05, 1000.0, 1000.0, ?, ?)",
            (now, now),
        )
        await db.commit()
        await take_phase0_baseline_snapshot(db)
