...
class PolymarketExecutionClientV2(BaseExecutionClient):
    ...
    async def submit_order(self, leg: OrderLeg, signal_id=None, strategy=None) -> OrderResult:
        ...
        try:
            ...
            response = await self._call(
                ...
                PartialCreateOrderOptions(tick_size=str(tick)), OrderType.GTC,
            )
    ...
    async def _poll(self, order_id, leg, start, max_polls=40):
        for _ in range(max_polls):
            ...
            if status in {"MATCHED", "UNMATCHED", "CANCELED", "CANCELLED"}:
                ...
                result = OrderResult(order_id=order_id, platform="polymarket", status="failed",
                                     submission_latency_ms=int((time.time()-start)*1000), error_message=f"terminal status={status}")
                await self.update_order_fill(result)
                return result
        await self.cancel_order(order_id)
