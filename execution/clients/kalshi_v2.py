...
class KalshiExecutionClientV2(BaseExecutionClient):
    ...
    async def _get_json(self, path):
        await self._limit()
        response = await self.http_client.get(self.api_base + path, headers=self._sign("GET", path))
    ...
    async def get_pretrade_fee_rate(self, leg):
        ...
        if fee_type not in {"quadratic", "quadratic_with_maker_fees"}:
            raise ValueError(f"unsupported Kalshi fee type for Phase 1: {fee_type!r}")
    ...
    async def submit_order(self, leg, signal_id=None, strategy=None):
        ...
        try:
            ...
            body = {"ticker": leg.market_id, "client_order_id": cid, "side": "bid" if leg.side.value == "BUY" else "ask", "count": f"{leg.size:.2f}", "price": f"{leg.limit_price:.4f}", "time_in_force": "immediate_or_cancel", "self_trade_prevention_type": "taker_at_cross", "cancel_order_on_pause": True}
            ...
            await self.write_order(leg, OrderResult(order_id=oid, platform="kalshi", status="pending", submission_latency_ms=int((time.time() - start) * 1000), fee_verified=False), signal_id=signal_id, strategy=strategy)
            return await self._poll(oid, leg, start)
        except Exception as exc:
            result = OrderResult(order_id=f"FAILED-{leg.market_id}", platform="kalshi", status="failed", submission_latency_ms=int((time.time() - start) * 1000), error_message=str(exc))
    ...
    async def _poll(self, oid, leg, start, max_polls=30):
        for _ in range(max_polls):
            ...
            response = await self.http_client.get(self.api_base + f"/portfolio/orders/{oid}", headers=self._sign("GET", path))
            if response.status_code != 200:
                continue
            ...
            if status in {"executed", "filled", "canceled", "cancelled"}:
                if matched > 0:
                    ...
                    result = OrderResult(order_id=oid, platform="kalshi", status="filled" if matched >= leg.size else "partially_filled", submission_latency_ms=int((time.time() - start) * 1000), fill_latency_ms=int((time.time() - start) * 1000), filled_price=price, filled_size=matched, fee_paid=fee, fee_verified=True)
                    await self.update_order_fill(result)
                    await self.write_fill_event(result)
                    return result
                result = OrderResult(order_id=oid, platform="kalshi", status="failed", submission_latency_ms=int((time.time() - start) * 1000), error_message="order cancelled without fill", fee_verified=True)
                await self.update_order_fill(result)
                return result
        await self.cancel_order(oid)
        return OrderResult(order_id=oid, platform="kalshi", status="pending", submission_latency_ms=int((time.time() - start) * 1000), error_message="fill poll timeout; order cancelled and requires reconciliation", fee_verified=False)

    async def cancel_order(self, oid):
        await self._limit()
        path = f"/trade-api/v2/portfolio/events/orders/{oid}"
        response = await self.http_client.delete(self.api_base + f"/portfolio/events/orders/{oid}", headers=self._sign("DELETE", path))
    ...
    async def get_balance(self):
        try:
            ...
            response = await self.http_client.get(self.api_base + "/portfolio/balance", headers=self._sign("GET", path))
            if response.status_code != 200:
                return None
