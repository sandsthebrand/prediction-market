...
def _assert_phase1_live_preflight():
    ...
    if missing:raise RuntimeError("Live execution blocked: set verified preflight flags: "+", ".join(missing))
...
def _make_execution_clients(db,execution_mode:str)->tuple[BaseExecutionClient,BaseExecutionClient]:
    ...
    return PaperExecutionClient(db,platform_label="polymarket"),PaperExecutionClient(db,platform_label="paper_kalshi")
