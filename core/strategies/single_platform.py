...
async def detect_single_platform_opportunities(
    ...
    for opp in opportunities:
        ...
        if result.filled_price is not None:
            ...
            # Commit per trade: a crash between fills and the final commit drops DB
            # records for positions already open on the exchange, which reconciliation
            # cannot repair. SQLite in WAL mode handles single-row commits cheaply.
            try:
                await db.commit()
            except Exception:
                logger.exception(
                    "Failed to commit position for strategy=%s market=%s pos=%s",
                    strategy,
                    m["id"],
                    pos_id,
                )

    # Final commit (no-op if all trades already committed; covers the case
    # where the last iteration had result.filled_price=None).
    await db.commit()
    return trades
