...
async def main():
    ...
    try:
        ...
        _snapshot_interval = cfg.observability.pnl_snapshot_interval_s
        ...
        async def _log_status():
            ...
            while not stop_event.is_set():
                try:
                    ...
                    logger.info(
                        "STATUS: pairs=%d eligible=%d muted=%d session_pnl=$%.2f "
                    ...
                    for _platform, _age_ms in stats[
                        "ws_last_tick_age_ms_by_platform"
                    ].items():
                        if _age_ms > 60_000:
                            logger.warning(
                                "STALE_WS_FEED platform=%s age_ms=%d",
                                _platform,
                                _age_ms,
                            )
                    now_ts = time.time()
